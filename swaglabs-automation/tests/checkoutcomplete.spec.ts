...
test('complete checkout with multiple items', async ({ page }) => {
  ...
  expect(await checkoutComplete.getCompleteHeaderText()).toBe('Thank you for your order!');
  expect(await checkoutComplete.getCompleteMessageText()).toBe('Your order has been dispatched, and will arrive just as fast as the pony can get there!');

  // verify back home button and return to inventory page
